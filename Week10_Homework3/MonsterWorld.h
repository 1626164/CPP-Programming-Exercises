...
public:
	...
	void play(int maxwalk, int wait) {
		...
		for (int i = 0; i < maxwalk; i++) {
			...
			for (int k = 0; k < nMon-2; k++)//변경
				pMon[k]->move(world.Data(), xMax, yMax);
			if (kbhit()) {//추가
				unsigned char ch = getche();//추가
				if (ch == 224) {//추가
					ch = getche();//추가
					((Tuman*)pMon[nMon-1])->move(world.Data(), xMax, yMax, ch);//추가
				}//추가
				else ((Tuman*)pMon[nMon-2])->moveHuman(world.Data(), xMax, yMax, ch);//추가
			}//추가
			nMove++;
			print();
			if (isDone()) break;
